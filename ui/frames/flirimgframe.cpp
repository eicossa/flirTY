...
void flirImgDisplayerGraphicsView::mouseMoveEvent(QMouseEvent *event)
{
    ...
    text = QString("%1 x %2").arg(origin.x()).arg(origin.y());
    sceneMousePos = this->mapToScene(event->pos());
...
#include "flirdata/flirtags.h"

flirImgFrame::flirImgFrame(const QString &name,
                           flirImg *fimg,
                           QWidget *parent)
    : QFrame(parent)
{
    ...
    this->ftags     = new flirtags("/home/eicossa/Data/nabtd/new/20160512_011655_303_R.JPG");
    this->fdata     = new flirmat(this->ftags);
...
void flirImgFrame::updateLCDDisplays()
{
    if(!isMouseOverImage){
        ...
        if(isLeafDetectionMode){
            ...
            lcdDisplay1->display(this->fdata->ftemp2->get_raw_at(mousePosRowNum, mousePosColNum));
            lcdDisplay2->display(this->fdata->ftemp2->get_temp_at(mousePosRowNum, mousePosColNum));

            // lcdDisplay1->display("asdf");
            // lcdDisplay2->display("bfda");
        }
    }
}
...
void flirImgFrame::displayPointInfoOnStatusBar(QPoint imgPos)
{
    ...
    if(mousePosColNum < 0 || mousePosRowNum < 0 || mousePosColNum > imgWidth || mousePosRowNum > imgHeight){
      ...
      if(isLeafDetectionMode){
        ...
        statusBar->showMessage(fdata->get_status_string(mousePosRowNum, mousePosColNum));
        updateLCDDisplays();
      }
    }
}
